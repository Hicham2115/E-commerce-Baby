import { extractGenresFromOptions, fetchCatalog } from "@/lib/shopify/catalog";
import {
  ...
  const match = variants.find((variant) => {
    if (selections.genre) {
      const hasGenre = variant.selectedOptions.some(
        (opt) =>
          normalize(opt.name) === "genre" &&
          normalize(opt.value) === normalize(selections.genre),
      );
    ...
    if (selections.size) {
      const hasSize = variant.selectedOptions.some(
        (opt) =>
          (normalize(opt.name) === "taille" ||
            normalize(opt.name) === "size") &&
          normalize(opt.value) === normalize(selections.size),
      );
