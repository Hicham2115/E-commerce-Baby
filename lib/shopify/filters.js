import { normalizeFilterValue as normalize } from "@/lib/shopify/metafield-values";

/** URL slug → Shopify variant option "Genre" value */
const GENRE_SLUG_TO_SHOPIFY = {
  fille: "Fille",
  garcon: "Garçon",
  unisexe: "Unisexe",
};
...
const CATEGORY_SLUG_TO_SHOPIFY = {
...
function resolveGenreShopifyValue(slug) {
  return GENRE_SLUG_TO_SHOPIFY[slug] ?? slug;
}

export function genreValueToSlug(value) {
...
function matchesGenre(product, genreSlug) {
...
function resolveCategoryShopifyName(slug) {
...
export function productsUrlByGenre(genre) {
  return buildProductsQuery({}, { genre });
}

export function buildProductsQuery(base, updates) {
