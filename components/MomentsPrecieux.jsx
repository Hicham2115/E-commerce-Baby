  ...
  return (
    <section id="univers" className="relative overflow-hidden bg-[#F9F8F6] px-4 py-16 md:px-8 md:py-24">
      ...
      <div className="relative mx-auto max-w-7xl">
        <FadeInUp className="mb-12 flex flex-col gap-6 md:mb-16 md:flex-row md:items-end md:justify-between">
        ...
        <StaggerContainer className="grid grid-cols-1 gap-4 md:grid-cols-12 md:gap-5" staggerDelay={0.08}>
          {tiles.map((tile) =>
            tile.isQuote ? (
              <StaggerItem key={tile.title} className={tile.className}>
                <Link
                  className="group relative flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-[#E8E4DC] bg-white p-6 shadow-[0_16px_40px_-12px_rgba(0,27,54,0.08)] transition-all duration-500 ease-out hover:-translate-y-1 hover:border-[#9B4D44]/30 hover:shadow-[0_24px_48px_-12px_rgba(0,27,54,0.12)] md:rounded-3xl md:p-8"
                  href={tile.href}
                >
              ...
              <StaggerItem key={tile.title} className={tile.className}>
                <Link
                  className="group relative block h-full overflow-hidden rounded-2xl shadow-sm transition-all duration-500 ease-out hover:-translate-y-1 hover:shadow-[0_20px_40px_-12px_rgba(0,27,54,0.2)] md:rounded-3xl"
                  href={tile.href}
                >
                  <Image
                    alt={tile.alt}
